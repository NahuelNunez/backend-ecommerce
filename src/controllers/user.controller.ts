...
export const getall = async (req: Request, res: Response) => {
  try {
    const user = await User.find({ eliminado: false });
...
export const edit = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Usuario editado exitosamente", update: users });
  } catch (error) {
...
export const destroyer = async (req: Request, res: Response) => {
  try {
    ...
    if (user) {
      res
        .status(200)
        .json({ messsage: "Usuario eliminado exitosamente", user: user });
    } else {
